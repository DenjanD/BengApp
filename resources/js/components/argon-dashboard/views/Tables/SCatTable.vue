<template>
  <div class="card shadow" :class="type === 'dark' ? 'bg-default': ''">
    <div class="card-header border-0" :class="type === 'dark' ? 'bg-transparent': ''">
      <div class="row align-items-center">
        <div class="col">
          <div class="row align-items-center">
            <div class="col-6">
              <h3 class="mb-0">Data Kategori Servis</h3>
            </div>
            <div class="col-6 text-right">
              ...
              <base-button type="primary" size="sm" @click="modals.modal_add_scat = true"><i class="ni ni-fat-add"
    ...
    <div v-show="showAlertAdd==true" class="alert alert-success alert-dismissible fade show" role="alert">
      ...
      <span class="alert-inner--text">Data kategori servis telah berhasil disimpan</span>
    ...
    <div v-show="showAlertUpdate==true" class="alert alert-success alert-dismissible fade show" role="alert">
      <span class="alert-inner--icon"><i class="notification-70"></i></span>
      <span class="alert-inner--text">Data kategori servis telah berhasil dirubah</span>
    ...
    <div v-show="showAlertDelete==true" class="alert alert-success alert-dismissible fade show" role="alert">
      <span class="alert-inner--icon"><i class="notification-70"></i></span>
      <span class="alert-inner--text">Data kategori servis telah berhasil dihapus</span>
    ...
    <div class="table-responsive">
      <table class="table align-items-center table-flush">
        ...
        <tr v-for="row in filteredDataShow">
          ...
          <td>
            <span v-show="editOffset != row.name">{{row.price}}</span>
            <input v-show="editOffset==row.name" class="form-control" type="text" :id="'item-user-'+row.price"
              v-model="editPost2">
          </td>
          <td>

            <span v-show="editOffset != row.name">
              <base-button type="success" size="sm" @click.prevent="startEditing(row.name,row.scategory_id);startEditing2(row.price)">
                <i class="fa fa-pen"></i> Ubah
              </base-button>
              <base-button type="danger" size="sm" @click="deleteData(row.scategory_id)">
    ...
    <!-- Modal goes here -->
    <modal :show.sync="modals.modal_add_scat">
      <template slot="header">
        <h5 class="modal-title" id="modal_add_scat">Tambah Kategori Servis Baru</h5>
      </template>
      <div>
        <label>Nama Kategori Servis Baru</label>
        <form role="form">
          <base-input v-model="modals.name" placeholder="Ketik nama di sini">
          </base-input>
        </form>
        <label>Biaya Kategori Servis Baru</label>
        <form role="form">
          <base-input v-model="modals.price" placeholder="Ketik biaya di sini">
          </base-input>
        </form>
      </div>
      <template slot="footer">
        <base-button type="secondary" @click="modals.modal_add_scat = false">Batal</base-button>
        <base-button type="primary" @click.prevent="addData()">Simpan Data</base-button>
      </template>
    </modal>

    <modal :show.sync="modals.modal_delete_scat">
      <template slot="header">
        <h5 class="modal-title" id="modal_delete_scat">Hapus Data Kategori Servis</h5>
      </template>
      <div>
        <label>Anda yakin ingin menghapus data kategori servis ini?</label>
      </div>
      <template slot="footer">
        <base-button type="secondary" @click="modals.modal_delete_scat = false">Batal</base-button>
  ...
  export default {
    data() {
      return {
        ...
        modals: { modal_add_scat: false, name: '', price:'', modal_delete_scat: false },
    ...
    methods: {
      loadData() {
        ...
        //Get scat data from database
        this.axios.get("api/scat", { headers: { Authorization: 'Bearer ' + localStorage.getItem('token') } }).then(response => {
      ...
      addData() {
        ...
        this.axios.post("api/scat", postData, reqConfig)
          .then(response => {
            //close modal
            this.modals.modal_add_scat = false
            this.loadData();
            this.modals.name = '',
            this.modals.price = '',
      ...
      updatePost(index) {
        ...
        this.axios.post("/api/scat/update", postData, reqConfig).then(response => {
          this.loadData()
        });
        this.editOffset = -1
        this.editPostOri = {}
      ...
      deleteDataConfirm() {
        ...
        this.axios.delete("/api/scat/" + this.deleteId, reqConfig).then(response => {
          this.loadData()
          this.showAlertDelete = true
          this.modals.modal_delete_scat = false
        });
      }
    },
    computed: {
      filteredDataShow() {
        return this.tableDataShow.filter((datas) => {
          ...
          if (this.searchFilter == 'Id Kategori Servis') {
            return datas.scategory_id.toString().match(this.searchKey)
          }
          if (this.searchFilter == 'Nama') {
            return datas.name.match(this.searchKey)
          }
          if (this.searchFilter == 'Biaya') {
            return datas.price.match(this.searchKey)
          }
        })
      }
    }
