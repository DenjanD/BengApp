<template>
  <div class="card shadow" :class="type === 'dark' ? 'bg-default': ''">
    <div class="card-header border-0" :class="type === 'dark' ? 'bg-transparent': ''">
      <div class="row align-items-center">
        <div class="col">
          <div class="row align-items-center">
            <div class="col-6">
              <h3 class="mb-0">Data Jasa Servis</h3>
            </div>
            <div class="col-6 text-right">
              <select class="cust-form w-40 mb-3" v-model="searchFilter">
                <option selected>--- Filter Pencarian ---</option>
                <option>Id Jasa</option>
                <option>Nama</option>
                <option>Biaya</option>
              </select>
              <base-button type="primary" size="sm" @click="modals.modal_add_job = true"><i class="ni ni-fat-add"
    ...
    <div v-show="showAlertAdd==true" class="alert alert-success alert-dismissible fade show" role="alert">
      ...
      <span class="alert-inner--text">Data job telah berhasil disimpan</span>
    ...
    <div v-show="showAlertUpdate==true" class="alert alert-success alert-dismissible fade show" role="alert">
      <span class="alert-inner--icon"><i class="notification-70"></i></span>
      <span class="alert-inner--text">Data job telah berhasil dirubah</span>
    ...
    <div v-show="showAlertDelete==true" class="alert alert-success alert-dismissible fade show" role="alert">
      <span class="alert-inner--icon"><i class="notification-70"></i></span>
      <span class="alert-inner--text">Data job telah berhasil dihapus</span>
        ...
        <tr v-for="row in filteredDataShow">
          ...
          <td>
            ...
            <span v-show="editOffset != row.name">
              <base-button type="success" size="sm" @click.prevent="startEditing(row.name,row.sjob_id);startEditing2(row.price);">
                <i class="fa fa-pen"></i> Ubah
              </base-button>
              <base-button type="danger" size="sm" @click="deleteData(row.sjob_id)">
    ...
    <!-- Modal goes here -->
    <modal :show.sync="modals.modal_add_job">
      <template slot="header">
        <h5 class="modal-title" id="modal_add_job">Tambah Job Baru</h5>
      </template>
      <div>
        <label>Nama Job Baru</label>
        <form role="form">
          <base-input v-model="modals.name" placeholder="Ketik nama di sini">
          </base-input>
        </form>
        <label>Biaya Job Baru</label>
        <form role="form">
      ...
      <template slot="footer">
        <base-button type="secondary" @click="modals.modal_add_job = false">Batal</base-button>
        <base-button type="primary" @click.prevent="addData()">Simpan Data</base-button>
      </template>
    </modal>

    <modal :show.sync="modals.modal_delete_job">
      <template slot="header">
        <h5 class="modal-title" id="modal_delete_job">Hapus Data Job</h5>
      </template>
      <div>
        <label>Anda yakin ingin menghapus data job ini?</label>
      </div>
      <template slot="footer">
        <base-button type="secondary" @click="modals.modal_delete_job = false">Batal</base-button>
  ...
  export default {
    data() {
      return {
        ...
        modals: { modal_add_job: false, name: '', price: '', modal_delete_job: false },
    ...
    methods: {
      loadData() {
        ...
        this.axios.get("api/sjob", { headers: { Authorization: 'Bearer ' + localStorage.getItem('token') } }).then(response => {
      ...
      addData() {
        ...
        this.axios.post("api/sjob", postData, reqConfig)
          .then(response => {
            //close modal
            this.modals.modal_add_job = false
            this.loadData();
            this.modals.name = '',
            this.modals.price = '',
      ...
      updatePost(index) {
        ...
        this.axios.post("/api/sjob/update", postData, reqConfig).then(response => {
          this.loadData()
        });
        this.editOffset = -1
        this.editPostOri = {}
      ...
      deleteDataConfirm() {
        ...
        this.axios.delete("/api/sjob/" + this.deleteId, reqConfig).then(response => {
          this.loadData()
          this.showAlertDelete = true
          this.modals.modal_delete_job = false
        });
      }
    },
    computed: {
      filteredDataShow() {
        return this.tableDataShow.filter((datas) => {
          ...
          if (this.searchFilter == 'Id Jasa') {
            return datas.sjob_id.toString().match(this.searchKey)
          }
          if (this.searchFilter == 'Nama') {
            return datas.name.match(this.searchKey)
          }
          if (this.searchFilter == 'Biaya') {
            return datas.price.toString().match(this.searchKey)
          }
        })
      }
    }
